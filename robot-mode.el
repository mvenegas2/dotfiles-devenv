...
(save-excursion
  (beginning-of-line)
  (if (string-match "^[ ]{4}" (thing-at-point 'line))
      (indent-rigidly (line-beginning-position) (line-end-position) (- 4))
    (if (string-match "^[^ ].*" (thing-at-point 'line))
        ()
      (indent-rigidly (line-beginning-position) (line-end-position) (- 7))
      )
    )
  )
)

;; (defun robot-mode-check-dots()
;; "Add four spaces after dots"
;; (interactive)
;; (if (string-match "^[ \t]+\.\.\.$" (thing-at-point 'line))
;;     (insert "    ")
;;   (insert " ")
...
"
  ...
  ;; (define-key robot-mode-map (kbd "<backtab>") 'robot-mode-deindent)
  (define-key robot-mode-map (kbd "RET") 'robot-mode-newline)
  ;; (define-key robot-mode-map (kbd "SPC") 'robot-mode-check-dots)
